...
app/
  ...
  seo-geo/page.tsx     — Beispiel einer eigenständigen Service-Landingpage
  standorte/page.tsx           — Übersicht aller Standorte
  standorte/[stadt]/page.tsx    — Übersicht aller Services für eine Stadt
  standorte/[stadt]/[service]/page.tsx — Eine Seite pro Stadt×Service-Kombination (automatisch generiert)
  robots.ts            — Generiert /robots.txt
  sitemap.ts           — Generiert /sitemap.xml (inkl. aller Standort-Seiten)
...
### Standort-Seiten (SEO für Stadt × Service)

Skalierbares System für lokale Landingpages wie "SEO in Balingen" oder
"Webdesign in Hechingen" — ohne pro Kombination eine Datei anzulegen.

**So fügst du eine neue Stadt hinzu:** einen Eintrag in `lib/locations.ts`
ergänzen (Slug, Name, Region, optionaler Kontext-Satz). Das war's — alle
Service-Seiten für diese Stadt entstehen automatisch:

```ts
{
  slug: "tuebingen",
  name: "Tübingen",
  region: "Landkreis Tübingen, Baden-Württemberg",
  proximityNote: "ca. 30 km von unserem Sitz in Haigerloch entfernt",
}
```

**So fügst du einen neuen Service hinzu:** einen Eintrag in
`lib/services.ts` ergänzen — erscheint danach automatisch für jede Stadt.

Für **3 Städte × 4 Services** entstehen so automatisch 12 Detailseiten plus
3 Stadt-Übersichtsseiten plus 1 Gesamtübersicht — alle mit eigenem Title,
eigener Meta-Description, eigenem `<h1>` und eigenen strukturierten Daten
(`Service`-Schema mit `areaServed`), und alle automatisch in `sitemap.ts`
gelistet.

**URL-Struktur:**
```
/standorte                    — Übersicht aller Städte
/standorte/balingen           — Übersicht aller Services für Balingen
/standorte/balingen/seo       — "SEO in Balingen"
/standorte/balingen/webdesign — "Webdesign in Balingen"
```

**Wichtig für echtes lokales SEO:** Die generierten Seiten nutzen dieselben
Textbausteine (`service.description()`) für alle Städte — das reicht als
technisches Grundgerüst, ist aber inhaltlich noch generisch. Google bewertet
rein templated Seiten ohne echten lokalen Mehrwert kritisch (Thin-Content-
/Duplicate-Content-Risiko). Für die wichtigsten Städte lohnt es sich,
zusätzlich echte lokale Inhalte zu ergänzen (Referenzen vor Ort, lokale
Keywords, ggf. Kundenstimmen) — das System hier ist die Basis, kein Ersatz
dafür.
